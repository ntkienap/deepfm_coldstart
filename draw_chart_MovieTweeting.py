...
df_feature_predictions_all = pd.read_csv(f"prediction_all/MovieTweeting/fm_feature({FEATURE_NAME})_MovieTweeting.csv",sep=",")
df_ratings_predictions_all = pd.read_csv(f"prediction_all/MovieTweeting/fm_ratings_MovieTweeting.csv",sep=",")
df_mf_predictions_all = pd.read_csv(f"prediction_all/MovieTweeting/mf_MovieTweeting.csv",sep=",")
...
df.to_csv(
    f"result_evaluation/all_model_feature({FEATURE_NAME})_MovieTweeting.csv", index=False)
plt.ylim(0, 0.4)
ax = sns.lineplot(
    data=df,
...
fig = ax.get_figure()
fig.savefig(
    f"evaluation/MovieTweeting/f1_feature({FEATURE_NAME})_MovieTweeting.png")
plt.close()
