...
if __name__ == "__main__":
    ...
    df_all_predictions.to_csv(
        f"prediction_all/mf_ml-{MOVIELENS_SIZE}_UMR({MIN_USER_RATING}).csv", index=False)
    print("Top K..........")
    evaluation = []
    x = range(1, 21)
    for k in x:
    ...
    df_eval.to_csv(
        f"result_evaluation/mf_ml-{MOVIELENS_SIZE}_UMR({MIN_USER_RATING}).csv", index=False)
    plt.ylim(0, 1)
    ...
    fig = ax.get_figure()
    name_fig = f"evaluation/mf_ml-{MOVIELENS_SIZE}_UMR({MIN_USER_RATING}).png"
    print(name_fig)
