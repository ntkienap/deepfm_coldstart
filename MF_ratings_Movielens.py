...
def main():
    parser = argparse.ArgumentParser(description="Matrix Factorization (SVD) on MovieLens")
    ...
    pred_out_file = f"prediction_all/mf_ml-{ml_size}_UMR({min_user_rating}).csv"
    ...
    eval_csv_file = f"result_evaluation/mf_ml-{ml_size}_UMR({min_user_rating}).csv"
    ...
    ax.set(title=f'ĐÁNH GIÁ MÔ HÌNH MF (SVD) - MovieLens {ml_size.upper()}')
    ax.set(xlabel='Top-K', ylabel='Score')
    fig = ax.get_figure()
    plot_file = f"evaluation/mf_ml-{ml_size}_UMR({min_user_rating}).png"
