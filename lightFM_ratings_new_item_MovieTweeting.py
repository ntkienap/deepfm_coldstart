...
PATH_FILE_LIGTHFM_MODEL = f"model/lightfm_with_ratings_new_item_MovieTweeting.model"
...
if __name__ == "__main__":
    ...
    df_all_predictions.to_csv(
        f"prediction_all/MovieTweeting/fm_ratings_new_item_MovieTweeting.csv", index=False)
    print("Top K..........")
    evaluation = []
    x = range(1, 21)
    for k in x:
    ...
    df_eval.to_csv(
        f"result_evaluation/MovieTweeting/fm_ratings_new_item_MovieTweeting.csv", index=False)
    plt.ylim(0, 1)
    ax = sns.lineplot(
        data=df_eval,
    ...
    fig = ax.get_figure()
    name_fig = f"evaluation/MovieTweeting/fm_ratings_new_item_MovieTweeting.png"
    print(name_fig)
