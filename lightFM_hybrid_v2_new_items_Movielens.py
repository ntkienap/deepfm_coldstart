...
def parse_args():
    parser = argparse.ArgumentParser(description="LightFM Hybrid (New Items) on MovieLens")
...
def main():
    ...
    path_file_lightfm_model = f"model/lightfm_with_feature({feature_name})_new_item_ml-{ml_size}_UMR({min_user_rating}).model"
    ...
    print(f"Loading MovieLens [{ml_size}] (New Item mode, min_user_rating={min_user_rating}, feature={feature_name}, dim={size_vector}, pooling={pooling})...")
    ...
    print("Predicting and evaluating Precision@k, Recall@k, F1@k for new items...")
    evaluation = []
    ...
    eval_csv = f"result_evaluation/fm_feature({feature_name})_new_item_ml-{ml_size}_UMR({min_user_rating}).csv"
    ...
    ax.set(title=f'LightFM Feature ({feature_name}) New Items - ml-{ml_size}', xlabel='TOP_K', ylabel='Score')
    fig = ax.get_figure()
    name_fig = f"evaluation/fm_feature({feature_name})_new_items_ml-{ml_size}_UMR({min_user_rating}).png"
