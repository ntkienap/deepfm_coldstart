...
PATH_FILE_LIGHTFM_MODEL = f"model/lightfm_with_feature({FEATURE_NAME})_new_item_ml-{MOVIELENS_SIZE}_UMR({MIN_USER_RATING}).model"
...
if __name__ == "__main__":
    ...
    df_eval.to_csv(
        f"result_evaluation/fm_feature({FEATURE_NAME})_new_item_ml-{MOVIELENS_SIZE}_UMR({MIN_USER_RATING}).csv", index=False)
    plt.ylim(0, 1.0)
    ax = sns.lineplot(
        data=df_eval,
    ...
    fig = ax.get_figure()
    name_fig = f"evaluation/fm_feature({FEATURE_NAME})_new_item_ml-{MOVIELENS_SIZE}_UMR({MIN_USER_RATING}).png"
    print(name_fig)
