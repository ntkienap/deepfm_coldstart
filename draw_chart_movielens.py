...
def main():
    parser = argparse.ArgumentParser(description="Draw evaluation charts for MovieLens models")
    ...
    pred_feat_path = f"prediction_all/fm_feature({feature_name})_ml-{ml_size}_UMR({min_user_rating}).csv"
    pred_rate_path = f"prediction_all/fm_ratings_ml-{ml_size}_UMR({min_user_rating}).csv"
    pred_mf_path = f"prediction_all/mf_ml-{ml_size}_UMR({min_user_rating}).csv"
    ...
    print(f"Charts saved successfully for ml-{ml_size}.")
